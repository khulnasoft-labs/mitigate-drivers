...
def new_mitidriver_page():
    st.title("Create a New MitiDriver")
    st.subheader(
        "Create a new MitiDriver yaml file quick and easy. Fill in as much details as possible and click Generate."
    )
...
def csv_viewer_and_searcher():
    ...
    if uploaded_file is not None:
        ...
        search_df["Id"] = search_df["Id"].apply(
            lambda id: f"https://www.mitigate-drivers.khulnasoft.com/drivers/{id}/"
        )
...
def main():
    st.set_page_config(page_title="MitiDriver")

    pages = {
        "Create a New MitiDriver": new_mitidriver_page,
